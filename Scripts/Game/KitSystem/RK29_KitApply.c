...
class RK29_KitApply
{
	...
	//------------------------------------------------------------------------------------------------
	//! Would `storage` take `prefab` at `slotId`, asked of the storage itself on either route and
	//! never through the manager: the manager cannot answer on a client (see s_bLocal), and two
	//! referees make the preview pack differently from the apply - a kit that fills its containers
	//! then fits on one machine and not the other. Placement stays the manager's where it has
	//! authority (SpawnInto), and a plan it refuses at the last moment reads as a drop like any
	//! other.
	//!
	...
	//! IsAdditionalWeightOk and CheckParentWeightLimit, all locally and on any machine. It is NOT
	//! honest for a mount, which inherits the base event unchanged - hence MountAccepts below.
	protected static bool CanTake(ResourceName prefab, BaseInventoryStorageComponent storage, int slotId)
	{
		if (!storage)
			return false;
	...
	//! The probe spawns on whichever machine is placing, the server included: a component that
	//! queues work in OnPostInit and cancels it in OnDelete only on the server throws on the
	//! delete - see RK29_RadioSourceComponent.
	...
	//------------------------------------------------------------------------------------------------
	//! CanTake for an item that already exists - the same ask on both routes, for the same reason.
	protected static bool CanTakeItem(notnull IEntity item, BaseInventoryStorageComponent storage,
		int slotId)
	{
		if (!storage)
			return false;

		// the item exists here, so the engine is asked without a probe - see MountAccepts
		if (slotId >= 0)
		{
			InventoryStorageSlot seat = storage.GetSlot(slotId);
			if (!seat || seat.GetAttachedEntity())
	...
	//! aside, re-issued at its new home) -> MakeRoomByRank (something less important comes out, one
	//! at a time and from one container) -> one last retry of everything displaced, with no right to
	//! displace in turn. Only then is it a drop.
	...
	protected static void BuildEligibility(notnull RK29_PlacementState st)
	{
	...
	protected static bool PlaceOne(SCR_InventoryStorageManagerComponent manager, notnull RK29_PlacementState st, int idx, bool mayDisplace)
	{
		...
		int chosen = ChooseContainer(st, idx);
		if (chosen == -1)
	...
	protected static int ChooseContainer(notnull RK29_PlacementState st, int idx)
	{
		ResourceName item = st.m_aItems[idx];
		...
		{
			if (!CanTake(item, st.m_aContainers[c], st.m_aSlotIds[c]))
				continue;
	...
	//------------------------------------------------------------------------------------------------
	//! Take one occupant out of container `c` and see whether that made room for item `idx`. True
	//! only when it did; an occupant that did not help goes back where it was, or the kit lands with
	//! its bandages in the ruck for an item that was dropped anyway. Only a container's own slots
	//! are candidates - GetAll recurses into pouches hung on a vest, and pulling an item out of one
	//! to "put it back" in the vest is a relocation nobody asked for.
	//!
	//! The occupant is re-issued, never moved: on the authority manager.TryInsertItemInStorage
	//! answers that the order was accepted while the item is still in `c` when the next line asks,
	//! so a trial move there proves nothing. A delete and a spawn both land in the frame that asks
	//! for them, which is what MakeRoomByRank already relies on. Do not put a move back; no state
	//! is at stake, every item this pass relocates was spawned by it.
	//!
	//! Only the first destination that will take the occupant is tried: the room freed in `c` is the
	//! occupant's own size whatever takes it, so no other destination can answer differently.
	protected static bool TryRelocate(SCR_InventoryStorageManagerComponent manager, notnull RK29_PlacementState st, int idx, int c, notnull IEntity occupant)
	{
		...
		// re-issuing needs the prefab, and an occupant that cannot name one must not be deleted
		ResourceName prefab = PrefabOf(occupant);
		if (prefab == ResourceName.Empty)
			return false;
		...
		for (int d = 0, n = st.m_aContainers.Count(); d < n; d++)
		{
			...
			if (CanTake(st.m_aItems[idx], st.m_aContainers[c], st.m_aSlotIds[c])
				&& Reissue(manager, st, moved, prefab, d, st.m_aSlotIds[d]))
			{
				RK29_Log.Trace("[RK29] evicted " + FilePath.StripPath("" + prefab)
					+ " to make room for " + FilePath.StripPath("" + st.m_aItems[idx]));
				return true;
			}

			// it did not help, or the destination refused it after all: back where it came from. An
			// occupant neither home nor away is handed to the displaced retry rather than lost - that
			// pass is the kit's own last chance, and failing it the item is reported as a drop.
			if (!Reissue(manager, st, moved, prefab, c, homeSlot))
			{
				Note(string.Format("[RK29] eviction undo failed - %1 is off the body",
					FilePath.StripPath("" + prefab)), LogLevel.WARNING);
				if (moved != -1)
				{
					st.m_aPlaced[moved] = false;
					st.m_aHome[moved] = -1;
					st.m_aSpawned[moved] = null;
					if (!st.m_aDisplaced.Contains(moved))
						st.m_aDisplaced.Insert(moved);
				}
			}
			return false;
		}
	...
	//------------------------------------------------------------------------------------------------
	//! A relocated occupant, spawned fresh into container `at` and recorded as the state's entity
	//! for `moved` - the old one is deleted, so a state that kept pointing at it would hand
	//! PickVictim a corpse. False when the container refused it after all.
	protected static bool Reissue(notnull SCR_InventoryStorageManagerComponent manager,
		notnull RK29_PlacementState st, int moved, ResourceName prefab, int at, int slotId)
	{
		IEntity fresh;
		if (!SpawnInto(manager, prefab, st.m_aContainers[at], slotId, fresh))
			return false;

		if (moved == -1)
			return true;

		// the manager's callback answers through replication, which a preview body has none of
		if (!fresh)
			fresh = NewestItemIn(st.m_aContainers[at], prefab, st.m_aSpawned);

		st.m_aPlaced[moved] = true;
		st.m_aSpawned[moved] = fresh;
		st.m_aHome[moved] = at;
		return true;
	}

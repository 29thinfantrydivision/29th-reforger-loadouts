...
//! decides which fields do anything: bare "group" reads only the two budget numbers,
//! "group/entry" only bounds.
...
class RK29_OverrideAdjust
{
	...
	[Attribute("-1", desc: "GROUP targets: new spend floor for a BUDGETED group (-1 = keep, 0 = no floor). Clamped to the budget this kit ends up with", category: "29th")]
	int m_iBudgetMin;
}
...
{
	[Attribute(desc: "What to look at: \"group/entry\", weapon-qualified as \"m16a3:optics/4x20\" where the group belongs to a gun. Naming a group without an entry counts everything in it; naming a group merged into a wider one through m_aIncludeGroups counts only what that group contributed. The reserved entry \"group/none\" watches for the group ANSWERING None - the bare seat, the empty weapon slot - which is how a rule keys on what the player did not take", category: "29th")]
	string m_sWhen;
	...
	[Attribute(desc: "What that rules out: \"group/entry\" for one option, or a bare \"group\" to rule out all of it. A group merged into a wider one through m_aIncludeGroups rules out only what it contributed - how a whole family is blocked without listing it, so a later addition to the family is covered. Same weapon-qualified form. The reserved entry \"group/none\" WITHHOLDS the bare answer, so a slot that may be left empty by some kits must be filled in this one; a bare \"group\" never takes None away, which is what makes FORCING None spellable as blocking everything else", category: "29th")]
	string m_sBlock;
}

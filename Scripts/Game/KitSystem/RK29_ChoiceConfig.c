...
class RK29_ChoiceGroup
{
	...
	[Attribute("0", desc: "Where this group sits in the detail column, and in the tile column. Higher SINKS; groups sharing a number keep the order they were offered in, which is the authored order for kit groups and the weapon order after them. Exists because the offer is built in resolution order - weapon groups arrive last - and that is not the order a player reads in. Utilities sits at the bottom this way. In the TILE column the weapon tiles occupy a band of their own around -50 (ordered by body slot, never by this number), so 0 and up - where every unauthored group sits - is below the guns and BELOW -50 is above them: that is how a group which governs what the guns may be gets read first", category: "29th")]
	int m_iOrder;
...
class RK29_ItemGroup : RK29_ChoiceGroup
{
	...
	[Attribute("0", desc: "BUDGETED: how much of the pool must be spent for the kit to be issued. 0 (the default) = no floor. The picker still lets a player drop below it while they trade rows around - Apply Kit is what refuses, naming the group. A minimum over the budget is clamped to it, and the authored defaults must reach it or every fresh kit would be refused", category: "29th")]
	int m_iBudgetMin;
...
//------------------------------------------------------------------------------------------------
//! A weapon slot the class fills. Entries are RK29_EntryWeapon. No slot field on purpose: the
//! winner's prefab declares its WeaponSlotType and the captured body says which slot carries that
//! type.
[BaseContainerProps(), RK29_ChoiceGroupTitle()]
class RK29_WeaponGroup : RK29_ChoiceGroup
{
	//! Same contract as the clothing flag - None is a deliberate pick, distinct from not answering -
	//! with one trap of its own: RK29_ResolvedGroup.DefaultEntry answers null for a group that allows
	//! None and flags no default, so a slot that must still arrive armed needs m_bDefault on the gun
	//! it arrives with.
	[Attribute("0", desc: "Taking nothing is a legal pick on this slot - rendered as a 'None' row. Off means the slot must be filled. A group that allows None and flags NO default entry issues no weapon by default, so flag the gun that should stay the standing answer. This is the AUTHORED offer; an exclusion blocking \"<group>/none\" withholds it per kit, so one class can be denied the empty slot another class may choose", category: "29th")]
	bool m_bAllowEmpty;
}

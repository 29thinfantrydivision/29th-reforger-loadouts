...
class RK29_WeaponGroup : RK29_ChoiceGroup
{
	...
	//! it arrives with.
	[Attribute("0", desc: "Taking nothing is a legal pick on this slot - rendered as a 'None' row. Off means the slot must be filled. A group that allows None and flags NO default entry issues no weapon by default, so flag the gun that should stay the standing answer", category: "29th")]
	bool m_bAllowEmpty;
}

...
class RK29_KitHud
{
	...
	//! The side the standing table was built for. A side change is invisible to the counts
	//! invoker - the counts are the same arrays on both sides of it, so RK29_SetCounts_S takes
	//! its equal-arrays early return and nothing fires - and the rows and title are filtered by
	//! side. Without this compare a reassigned player reads the other team's table all briefing.
	protected string m_sPaintedFactionKey;
	...
	protected void Tick()
	{
		...
		// see m_sPaintedFactionKey - the one change the counts invoker cannot announce
		if (RK29_KitHud.LocalFactionKey() != m_sPaintedFactionKey)
			m_bRowsPainted = false;
	...
	protected void Rebuild()
	{
		...
		m_bRowsPainted = true;
		m_sPaintedFactionKey = factionKey;
	}

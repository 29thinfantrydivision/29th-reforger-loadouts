...
class RK29_ResolvedEntry
{
	...
	//! The catalog group this entry arrived through by m_aIncludeGroups; empty for the group's own. An
	//! exclusion naming an included group's id reaches only these - RK29_KitResolve.TargetReaches. A
	//! duplicate id keeps its first arrival's origin, so a later include cannot claim it.
	string m_sFromGroup;

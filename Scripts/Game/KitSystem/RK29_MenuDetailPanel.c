...
class RK29_MenuDetailPanel
{
	...
	//! The seated plate when the row it sits on issues NONE: the gun starts empty. Two ways in, and
	//! the plate cannot tell them apart - the player marked this row while it was empty, or every row
	//! feeding the chamber is at zero. Any other zero row loses the mark to a stocked one
	//! (RK29_KitResolve.SeatedLoadedEntry), so this paint always means the chamber is empty and not
	//! merely that this row is. See StampLoadedToggle.
	...
	protected void StampCountedHeader(notnull RK29_ResolvedGroup g)
	{
		...
		RK29_HeaderSupply supply;
		string caption = CountedHeaderCaption(g, flags, supply);
		RK29_MenuRowKit.StampHeader(m_wColDetail, caption, flags, supply);
	}
	...
	{
		// NoneAllowed, not the authored flag: a rule that withheld the bare answer takes the row
		// away rather than greying it. There is no entry behind it to carry a culprit, so a greyed
		// row could not say who blocked it - and a row that ignores a click has to say why
		if (!g.NoneAllowed())
			return;
	...
	//------------------------------------------------------------------------------------------------
	//! "Blocks <item>." - the item, or the whole section where the exclusion names one. A family merged
	//! into the section is told by what it leaves: "Limits <section> to <item>." when one row survives,
	//! "Limits <section>." otherwise. Nothing more: the greyed row explains itself.
	protected string ExclusionNoteOf(notnull RK29_ResolvedEntry e)
	{
		...
		if (entryId == "" && target.m_sId == groupId)
			return "Blocks " + GroupLabel(groupId) + ".";

		RK29_ResolvedEntry victim = target.FindEntry(entryId);
		if (entryId != "")
		{
			if (!victim || !RK29_KitResolve.TargetReaches(target, victim, groupId, entryId))
				return "";
			return "Blocks " + m_Menu.EntryLabelIn(target, victim) + ".";
		}

		int reached;
		RK29_ResolvedEntry survivor;
		int survivors;
		foreach (RK29_ResolvedEntry candidate : target.m_aEntries)
		{
			if (!candidate)
				continue;
			if (RK29_KitResolve.TargetReaches(target, candidate, groupId, entryId))
			{
				reached++;
				continue;
			}
		...
		// a family with nothing for this side limits nothing
		if (reached == 0)
			return "";

		string section = target.m_sDisplayName;
		if (section == "")
			section = target.m_sId;
		if (survivors == 1)
			return "Limits " + section + " to " + m_Menu.EntryLabelIn(target, survivor) + ".";
	...
	//! seated row is bound by its authored floor alone: stepping it to zero is legal, and the mark
	//! then moves to the best-stocked row on that chamber - unless it was put there empty on purpose.
	...
	//! carries, and a LOADED row reading 0 means the gun starts empty.
		...
		bool isLoaded, int count, int index)
	{
		...
		// seated on a row held at zero means the whole chamber is dry - see LOADED_EMPTY. Only the
		// red case is painted: the row is built fresh every stamp, with the layout's amber
		if (isLoaded && count <= 0)
		{
	...
	//------------------------------------------------------------------------------------------------
	//! The caption above a counted group - its name only. The three budget numbers come back in
	//! outSupply because they sit on the opposite side of the header line, inside the supply zone.
	...
	protected string CountedHeaderCaption(notnull RK29_ResolvedGroup g,
		out RK29_EHeaderFlags outFlags, out RK29_HeaderSupply outSupply)
	{
		...
		outSupply = new RK29_HeaderSupply();
		outSupply.m_iSpend = GroupSpend(g);
		outSupply.m_iMin = g.m_iBudgetMin;
		outSupply.m_iMax = g.m_iBudget;
		return g.m_sDisplayName;
	}
	...
	//------------------------------------------------------------------------------------------------
	//! Which row of the chamber wears the mark; "" for no selector at all. Through
	//! RK29_KitResolve.SeatedLoadedEntry and not the raw pick, so the row painted LOADED is the row
	//! the apply chambers - including when the mark has moved off a row the kit stopped carrying.
	protected string SeatedEntryIdOf(RK29_ResolvedGroup loaded)
	{
		if (!loaded)
			return "";

		RK29_ResolvedEntry seated = RK29_KitResolve.SeatedLoadedEntry(m_Menu.Offer(), loaded,
			m_Menu.Picks());
		if (!seated)
			return "";
	...
	void OnLoadedToggle(int index)
	{
		...
		RK29_ResolvedEntry clicked = loaded.FindEntry(entryId);
		if (!clicked)
			return;

		// Marking a row the kit carries none of while the chamber has stock elsewhere is the player
		// asking for an empty chamber - kit the magazines, then move the mark onto the row held at
		// zero. Said here because only the click knows it: a moment later the pick is
		// indistinguishable from a mark whose row was emptied under it, which asks for the opposite.
		// A chamber with nothing on it at all is not flagged - it is empty whatever it names, and
		// the first row to be stocked should take the mark.
		bool chamberHasStock = RK29_KitResolve.FullestLoadedEntry(m_Menu.Offer(), loaded,
			m_Menu.Picks()) != null;
		bool rowIsEmpty = RK29_KitResolve.CarriedCountOfLoaded(m_Menu.Offer(), loaded, clicked,
			m_Menu.Picks()) <= 0;

		int count = 1;
		if (rowIsEmpty && chamberHasStock)
			count = RK29_KitResolve.LOADED_PICK_EMPTY;

		m_Menu.SetPick(loaded.m_sId, entryId, count);
	...
	void OnCountStep(int index, int step)
	{
		...
		// zero, and the mark leaves it for whatever is still stocked - or the apply pass clears the muzzle,
		// which is also what an empty chamber the player asked for rides out on

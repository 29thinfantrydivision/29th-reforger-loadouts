...
class RK29_ResolvedGroup
{
	...
	int m_iBudget;
	int m_iBudgetMin;        // BUDGETED: spend the kit must reach to be issued, 0 = no floor. Never above m_iBudget - RK29_KitResolve.ClampBudgetMin holds that
	...
	bool m_bNoneBlocked;     // a rule withheld that answer; m_bAllowEmpty stays as authored - ask NoneAllowed()
	...
	//! to "back" must still reach the superset that swallowed it (back_medic). Empty on a
	...
	//------------------------------------------------------------------------------------------------
	//! Is taking nothing an answer the player may still give? The authored flag says the group has a
	//! None row at all; an exclusion addressed to "<group>/none" withholds it per kit without
	//! touching the authoring. Every reader asks this rather than m_bAllowEmpty, because None is not
	//! an entry: nothing can mark it blocked, so a block that only cleared a flag on some row would
	//! grey the row and still issue the bare answer.
	bool NoneAllowed()
	{
		return m_bAllowEmpty && !m_bNoneBlocked;
	}
	...
	RK29_ResolvedEntry DefaultEntry()
	{
		...
		// a withheld None is not an answer to fall back on, so the group owes a real one
		if (NoneAllowed())
			return null;

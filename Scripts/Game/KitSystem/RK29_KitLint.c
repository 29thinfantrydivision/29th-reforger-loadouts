...
class RK29_KitLint
{
	...
	//------------------------------------------------------------------------------------------------
	//! A BUDGETED group whose authored defaults already overspend its budget is unusable, not just
	//! wrong: spend is measured with defaults standing in for unpicked rows, so the group is over
	//! budget with no picks at all, DropBlockedPicks then discards every pick of it on each rebuild,
	//! and the stepper's down-step lands back on the default - the player can neither raise nor
	//! lower a row. Per faction, because ugl_grenades authors a US and a USSR "he" row and only one
	//! reaches any player. Not covered: an override's m_aAdjust raising a default per kit.
	protected static int ComplainIfDefaultsOverspend(RK29_ChoiceGroup group, string where)
	{
	...
	protected static void VerifyBudgets(notnull RK29_KitSetup setup)
	{
		...
		foreach (RK29_ChoiceGroup g : setup.m_aChoiceGroups)
			bad += ComplainIfDefaultsOverspend(g, "catalog");
		...
		if (setup.m_aWeaponDefs)
		{
			...
			{
				if (wdef)
					bad += ComplainIfDefaultsOverspend(wdef.m_AmmoGroup,
						"weapon '" + wdef.m_sId + "' ammo");
			}
		}
		...
		CollectInlineGroups(setup, inlineGroups, inlineKits);
		foreach (int i, RK29_ChoiceGroup g : inlineGroups)
			bad += ComplainIfDefaultsOverspend(g, "kit '" + inlineKits[i] + "' inline group");

		if (bad > 0)
			Print(string.Format("[RK29] config ERROR - %1 budgeted group(s) overspend their own"
				+ " budget by default", bad), LogLevel.ERROR);
	}

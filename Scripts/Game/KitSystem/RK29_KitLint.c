...
class RK29_KitLint
{
	...
	protected static void VerifyExclusions(notnull RK29_KitSetup setup)
	{
		...
		foreach (RK29_ClassSetup cls : setup.m_aClasses)
		{
			...
			foreach (RK29_Exclusion exclusion : comp.m_aExclusions)
			{
				...
				bad += ComplainIfNoneTargetIsWrong(setup, cls, exclusion.m_sWhen, "watches");
				bad += ComplainIfNoneTargetIsWrong(setup, cls, exclusion.m_sBlock, "blocks");
		...
		notnull RK29_ClassSetup cls, string path, string role)
	{
		...
		if (FindAuthoredGroup(setup, cls, groupId))
			return 0;
	...
	//------------------------------------------------------------------------------------------------
	//! The authored definition a target names: a catalog group, or one the kit declares inline.
	//! Both are legal targets. Null when the kit offers no such group. A *Ref names a catalog group,
	//! which the catalog lookup already answers for, so refs are skipped.
	protected static RK29_ChoiceGroup FindAuthoredGroup(notnull RK29_KitSetup setup,
		notnull RK29_ClassSetup cls, string groupId)
	{
		RK29_ChoiceGroup found = setup.FindChoiceGroup(groupId);
		if (found)
			return found;
	...
	//------------------------------------------------------------------------------------------------
	//! Does this authored group offer a None row at all? The flag lives on the four subclasses that
	//! have one rather than on the base, so the answer is four casts and not a field read.
	protected static bool GroupAllowsNone(RK29_ChoiceGroup def)
	{
		RK29_WeaponGroup weapon = RK29_WeaponGroup.Cast(def);
		if (weapon)
			return weapon.m_bAllowEmpty;

		RK29_AttachmentGroup attachment = RK29_AttachmentGroup.Cast(def);
		if (attachment)
			return attachment.m_bAllowEmpty;

		// a garment-attachment group is a sibling of the clothing class, never both - read the
		// narrower one first, exactly as CopyGroupShape does
		RK29_GarmentAttachmentGroup garment = RK29_GarmentAttachmentGroup.Cast(def);
		if (garment)
			return garment.m_bAllowEmpty;

		RK29_ClothingGroup worn = RK29_ClothingGroup.Cast(def);
		if (worn)
			return worn.m_bAllowEmpty;

		return false;
	}

	//------------------------------------------------------------------------------------------------
	//! An adjust changes a ROW's bounds, and None is not a row - so "<group>/none" as an adjust
	//! target is always a silent no-op (ApplyOverride's FindEntry answers null and the step is
	//! skipped). It is the natural way to reach for "make None the default", so it is named here
	//! with the spelling that works instead.
	protected static int ComplainIfAdjustTargetsNone(notnull RK29_ClassSetup cls, string path)
	{
		string weaponId, groupId, entryId;
		RK29_KitResolve.SplitTarget(path, weaponId, groupId, entryId);
		if (entryId != RK29_KitResolve.NONE_ENTRY)
			return 0;

		Print(string.Format("[RK29] config ERROR - kit '%1' adjusts '%2', but None is not a row and"
			+ " has no bounds to change - to make None the standing answer, clear m_bDefault on the"
			+ " group's entries; to withhold it, use an exclusion blocking '%3/none'",
			cls.m_sKitName, path, groupId), LogLevel.ERROR);
		return 1;
	}

	//------------------------------------------------------------------------------------------------
	//! "<group>/none" addresses the bare answer rather than a row (RK29_KitResolve.NONE_ENTRY).
	//! Two ways to write it and mean nothing, both silent at runtime because None carries no entry
	//! to fall back on: naming it on a group that offers no None, and blocking it on a group whose
	//! only answer it is. The near-miss is worth the check on its own - "optics_none" is a GROUP,
	//! "optics/none" is this, and they are one character apart.
	protected static int ComplainIfNoneTargetIsWrong(notnull RK29_KitSetup setup,
		notnull RK29_ClassSetup cls, string path, string role)
	{
		string weaponId, groupId, entryId;
		RK29_KitResolve.SplitTarget(path, weaponId, groupId, entryId);
		...
		// a group this kit never offers is ComplainIfNoSuchGroup's complaint, not a second one
		RK29_ChoiceGroup def = FindAuthoredGroup(setup, cls, groupId);
		if (!def)
			return 0;

		if (!GroupAllowsNone(def))
		{
			Print(string.Format("[RK29] config ERROR - kit '%1' %2 '%3', but group '%4' offers no"
				+ " None to address - set m_bAllowEmpty on it, or the rule can never take effect",
				cls.m_sKitName, role, path, groupId), LogLevel.ERROR);
			return 1;
		}

		if (role == "blocks" && (!def.m_aEntries || def.m_aEntries.IsEmpty()))
		{
			Print(string.Format("[RK29] config ERROR - kit '%1' blocks '%2', but group '%3' has no"
				+ " entries, so None is its whole offer - blocking it leaves the group with no legal"
				+ " answer", cls.m_sKitName, path, groupId), LogLevel.ERROR);
			return 1;
		}
	...
	//------------------------------------------------------------------------------------------------
	//! Both ways an authored budget leaves a group unusable before anyone has picked anything.
	//!
	//! Over the ceiling: spend is measured with defaults standing in for unpicked rows, so the group
	//! is over budget with no picks at all, DropBlockedPicks then discards every pick of it on each
	//! rebuild, and the stepper's down-step lands back on the default - the player can neither raise
	//! nor lower a row.
	//!
	//! Under the floor: the defaults are what an untouched kit and the Standard preset carry, so a
	//! floor they do not reach refuses every fresh kit at Apply, and the player is told to raise
	//! something in a group they never touched. A floor above the ceiling is the same mistake
	//! stated twice - the runtime clamps it, the authoring is still wrong.
	//!
	//! Per faction, because ugl_grenades authors a US and a USSR "he" row and only one
	//! reaches any player. Not covered: an override's m_aAdjust moving a default or a budget per kit.
	protected static int ComplainIfBudgetUnworkable(RK29_ChoiceGroup group, string where)
	{
		RK29_ItemGroup g = RK29_ItemGroup.Cast(group);
		if (!g || !g.m_bEnabled || g.m_eKind != RK29_EChoiceKind.BUDGETED || !g.m_aEntries)
			return 0;

		int badPair = 0;
		if (g.m_iBudgetMin > g.m_iBudget)
		{
			Print(string.Format("[RK29] config ERROR - %1 group '%2' authors a minimum of %3 against"
				+ " a budget of %4; nothing could ever spend that much and the floor is clamped to"
				+ " the budget at runtime", where, g.m_sId, g.m_iBudgetMin, g.m_iBudget),
				LogLevel.ERROR);
			badPair = 1;
		}
		...
		foreach (RK29_ChoiceEntryBase e : g.m_aEntries)
		{
			if (!e || !e.m_bEnabled || !e.m_aFactionKeys)
				continue;
			foreach (string key : e.m_aFactionKeys)
			{
		...
		foreach (string key : keys)
		{
			...
			foreach (RK29_ChoiceEntryBase e : g.m_aEntries)
			{
				...
				if (!item || !item.m_bEnabled)
					continue;
				if (key != "" && item.m_aFactionKeys && !item.m_aFactionKeys.IsEmpty()
					&& !item.m_aFactionKeys.Contains(key))
					continue;
				spend += Math.MaxInt(item.m_iDefault, item.m_iMin) * item.m_iCost;
			}
			...
			if (spend > g.m_iBudget)
			{
				Print(string.Format("[RK29] config ERROR - %1 group '%2' authors defaults spending %3"
					+ " against a budget of %4 (%5); the group is over budget before any pick and no"
					+ " row of it can be stepped in either direction",
					where, g.m_sId, spend, g.m_iBudget, side), LogLevel.ERROR);
				found++;
				continue;
			}

			// against the clamped floor, so a group that already drew the pair complaint above is not
			// measured against a number the runtime will never use
			int floor = Math.MinInt(g.m_iBudgetMin, g.m_iBudget);
			if (floor > 0 && spend < floor)
			{
				Print(string.Format("[RK29] config ERROR - %1 group '%2' authors defaults spending %3"
					+ " against a minimum of %4 (%5); an untouched kit of this class would be refused"
					+ " at Apply", where, g.m_sId, spend, floor, side), LogLevel.ERROR);
				found++;
			}
		}
		return found;
	...
	//------------------------------------------------------------------------------------------------
	//! Same three authoring homes as VerifyDuplicateEntries.
	protected static void VerifyBudgets(notnull RK29_KitSetup setup)
	{
		int bad = 0;

		foreach (RK29_ChoiceGroup g : setup.m_aChoiceGroups)
			bad += ComplainIfBudgetUnworkable(g, "catalog");
		...
		if (setup.m_aWeaponDefs)
		{
			...
			{
				if (wdef)
					bad += ComplainIfBudgetUnworkable(wdef.m_AmmoGroup,
						"weapon '" + wdef.m_sId + "' ammo");
			}
		}
		...
		CollectInlineGroups(setup, inlineGroups, inlineKits);
		foreach (int i, RK29_ChoiceGroup g : inlineGroups)
			bad += ComplainIfBudgetUnworkable(g, "kit '" + inlineKits[i] + "' inline group");

		if (bad > 0)
			Print(string.Format("[RK29] config ERROR - %1 budgeted group(s) cannot be satisfied as"
				+ " authored", bad), LogLevel.ERROR);
	}
			...
			foreach (RK29_Override p : overrides)
			{
				if (p.m_aAdjust)
				{
					foreach (RK29_OverrideAdjust adj : p.m_aAdjust)
					{
						...
						bad += ComplainIfAdjustTargetsNone(cls, adj.m_sTarget);
					}
				}
				if (p.m_aAddGroups)
				{

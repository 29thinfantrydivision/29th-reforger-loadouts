...
class RK29_KitManager
{
	...
	//! Backstop behind the player events the tally is rebuilt from, in PHASE_WATCH_MS ticks. The
	//! truth Recompute_S reads can change with nothing to hear it: a reconnect onto a reserved
	//! body is not a spawn, a deploy-menu row click reassigns the loadout through no invoker this
	//! mod holds, and a GM possession swaps the controlled entity outright. Every one of those
	//! shows a wrong number on every screen until the next unrelated event, so the tally is
	//! re-derived from scratch on this cadence and a correction is logged.
	//! DELETE this and the sweep when a full briefing logs no "count sweep corrected" line.
	protected static const int RECOMPUTE_SWEEP_TICKS = 10;
	...
	//! Sweep bookkeeping: the tick counter, and whether the rebuild about to run is the sweep's
	//! own. Only set when no event had already queued one, so a correction the log blames on the
	//! sweep really is one the events missed.
	protected int m_iSweepTicks_S;
	protected bool m_bSweepRecompute_S;
	...
	{
		// The outgoing manager must let go first: the game's call queue outlives the world, so a
		// timer it left armed would keep a dead manager alive and ticking.
		if (s_Instance)
	...
	void HandleKitRequest_S(int playerId, string kitName, string choices)
	{
		...
		// the picker refuses to send a kit that leaves a pool under its floor, so a wire that says
		// otherwise is not one this server issues. Dropped rather than clamped: the player's own menu
		// has already told them which group is short, and quietly issuing something else would put
		// gear on the body that nobody chose
		string shortGroup = RK29_KitResolve.FirstUnderMinGroup(cls, m_Setup, sel.m_aPicks);
		if (shortGroup != "")
		{
			Print(string.Format("[RK29] kit request refused - '%1' picks leave group '%2' under its"
				+ " minimum (player %3)", kitName, shortGroup, playerId), LogLevel.WARNING);
			return;
		}
	...
	//------------------------------------------------------------------------------------------------
	//! A client's remembered kit, offered once its side is known. Served only where this server
	//! holds NOTHING for the player, or holds a selection from the other side - anything else is a
	//! reconnect's parked stash or a kit they asked for this session, and both outrank a profile
	//! file. That test is the only thing that tells a fresh session from a rejoin: the client cannot,
	//! because RK29_LocalStash.Clear() wipes its copy on every world rebuild and leaving a server is
	//! one. Everything past the guard is an ordinary request, re-validated and throttled like any
	//! other - a seed names a kit the player could have asked for by hand.
	void HandleKitSeed_S(int playerId, string kitName, string choices)
	{
		if (!Replication.IsServer())
			return;

		RK29_PlayerSelection held = m_mSelections.Get(playerId);
		if (held)
		{
			RK29_KitStruct heldKit;
			if (m_mKits)
				heldKit = m_mKits.Get(held.m_sKitName);
			Faction faction = SCR_FactionManager.SGetPlayerFaction(playerId);

			// only a positively confirmed same-side selection wins; one naming a kit this build no
			// longer has is stale, and the seed is a better answer than a dangling name
			if (heldKit && faction && heldKit.m_sFactionKey == faction.GetFactionKey())
			{
				RK29_Log.Trace(string.Format("[RK29] kit seed ignored - player %1 already has"
					+ " '%2' on this side", playerId, held.m_sKitName));
				return;
			}
		}

		HandleKitRequest_S(playerId, kitName, choices);
	}
	...
	//------------------------------------------------------------------------------------------------
	//! Counts are rebuilt on the edge into briefing, the one moment no player event marks, and
	//! then swept every RECOMPUTE_SWEEP_TICKS. Not every tick: a rebuild per second would cost a
	//! full offer build per living player every second. Not the edge alone either - the player
	//! events do not cover every change inside the phase, which is what the sweep is for.
	protected void WatchPhase_S()
	{
		...
		{
			m_iSweepTicks_S++;
			if (m_iSweepTicks_S >= RECOMPUTE_SWEEP_TICKS)
			{
				m_iSweepTicks_S = 0;
				// piggybacking on a rebuild an event already asked for is not a missed event
				m_bSweepRecompute_S = !m_bRecomputeQueued_S;
				QueueRecompute_S();
			}
		}
	...
	{
		bool sweep = m_bSweepRecompute_S;
		m_bSweepRecompute_S = false;
		...
		// a sweep that changes anything found a truth change no player event announced - the line
		// is the sweep's own deletion condition, see RECOMPUTE_SWEEP_TICKS
		if (gm.RK29_SetCounts_S(alive, magnified) && sweep)
			Print("[RK29] count sweep corrected the kit tally - a change reached it with no event"
				+ " (reconnect onto a held body, a deploy-menu loadout request, or a possession)",
				LogLevel.WARNING);
	}

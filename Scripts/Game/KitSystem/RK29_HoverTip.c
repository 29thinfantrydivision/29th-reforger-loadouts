...
class RK29_HoverTip
{
	...
	//! The panel sizes to its content on both axes, so a wrapped tip never clips. keepLines
	//! bypasses the budget entirely, which makes it the CALLER's to keep: a line longer than this
	//! carries the panel past the layer's right edge and is cut there, because the clamp below
	//! measures a panel the engine has not re-laid-out for the new words yet.

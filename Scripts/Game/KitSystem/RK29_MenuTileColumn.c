...
class RK29_MenuTileColumn
{
	...
	//! Where the weapon tiles sit in the one order the whole column is placed by. A weapon group is
	//! ordered by the BODY SLOT it claimed, not by an authored number: the slot comes from the
	//! winning gun's prefab, so a kit that swaps its rifle keeps PRIMARY above SIDEARM for free and
	//! no kit has to author an order on its weapon groups. This band is what places that block
	//! against everything else - a group authored BELOW it sits above the weapons, and 0 and up
	//! sits below, which is where every unauthored group already lands.
	protected static const int WEAPON_BAND = -50;

	//! The slotless sink, one past the sidearm. Without it a weapon group that claimed no slot takes
	//! ClaimedSlotOf's NO_SLOT (9999) into the sum and sorts under the gear instead of after the
	//! guns.
	protected static const int WEAPON_BAND_SINK = 3;
	...
	void BuildTiles()
	{
		...
		// one list and one order: a weapon group is placed by its slot inside WEAPON_BAND and
		// everything else by its authored m_iOrder, so a group that governs what sits under it can
		// be authored above the guns instead of being stuck below them by the stamping order
		array<RK29_ResolvedGroup> tiles = {};
		...
		SortTiles(tiles, slots);

		foreach (RK29_ResolvedGroup g : tiles)
		{
			if (g.IsWeaponGroup())
			{
				int slot = RK29_MenuRowKit.ClaimedSlotOf(g, slots);
				...
				RK29_MenuRowKit.StampHeader(m_wColTiles, RK29_MenuRowKit.SlotCaptionOf(slot),
					RK29_MenuRowKit.LockFlag(WeaponTileLocked(g)));
				StampWeaponTile(g, slot);
				continue;
			}
			...
			if (g.IsGarmentAttachmentGroup())
			{
				if (g.m_bSignpostInLoadout)
					StampGarmentLink(g);
	...
	//! Bare answers included: a slot the player emptied keeps its caption and its place in the
	//! column rather than dropping to the bottom as an unnamed "WEAPON".
	...
	//! Where one tile sits in the column: a weapon group by the body slot it claimed, inside
	//! WEAPON_BAND; everything else by the order its config authored. Deliberately NOT written back
	//! onto m_iOrder - the detail column reads that for its own sections, and a group means one
	//! thing there and another here.
	protected int TileOrderOf(RK29_ResolvedGroup g, notnull map<string, int> slots)
	{
		if (!g)
			return 0;
		...
		int slot = RK29_MenuRowKit.ClaimedSlotOf(g, slots);
		if (slot < 0 || slot > WEAPON_BAND_SINK)
			slot = WEAPON_BAND_SINK;

		return WEAPON_BAND + slot;
	}

	//------------------------------------------------------------------------------------------------
	//! Body-slot order for the guns - primary, secondary, sidearm, then whatever claimed no slot -
	//! and authored order for the rest, in one pass. Insertion sort because it is stable: groups
	//! sharing a number keep offer order instead of swapping per rebuild, the same reason
	//! RK29_KitResolve.SortOfferByOrder is one.
	protected void SortTiles(notnull array<RK29_ResolvedGroup> groups,
		notnull map<string, int> slots)
	{
		for (int i = 1, n = groups.Count(); i < n; i++)
		{
			RK29_ResolvedGroup moving = groups[i];
			int key = TileOrderOf(moving, slots);

			int j = i - 1;
			while (j >= 0 && TileOrderOf(groups[j], slots) > key)
			{

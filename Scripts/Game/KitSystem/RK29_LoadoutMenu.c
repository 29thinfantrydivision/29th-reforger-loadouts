...
class RK29_LoadoutMenu
{
	...
	//! The heading over a refused Apply; the lines under it name what to fix, at most this many
	//! before the rest are counted off - see TrimReasons.
	protected static const string APPLY_ERROR_TITLE = "Can't apply kit";
	protected static const int APPLY_ERROR_MAX_LINES = 8;
	...
	//! The refusal band along the bottom of the content, raised only by RefuseApply and taken down
	//! by the next rebuild. Held rather than looked up per press: it belongs to no column, so no
	//! panel owns it.
	protected Widget m_wApplyError;
	...
	protected ref RK29_PresetMenu m_PresetMenu = new RK29_PresetMenu();
	protected ref RK29_PresetDrag m_PresetDrag = new RK29_PresetDrag();

	//! Which mouse button pressed the row whose click is now being dispatched, noted an instant
	//! earlier by that row's own handler. The click event cannot answer it: the component takes
	//! the button and drops it, and m_OnClicked carries no trace. Read and cleared together, so
	//! the note can never outlive its own click.
	...
	protected void BindWidgets()
	{
		...
		m_wApplyError = RK29_WidgetUtil.Require(m_wRoot, "ApplyError");

		m_Tiles.Init(this, colClasses, colTiles);
		m_Detail.Init(this, colDetail, detailTitle, detailTitleBand);
		m_Info.Init(this, colInfo);

		// all three find their own widgets off the root handed to them here, and all three are given
		// back
		m_HoverTip.Bind(m_wRoot);
		m_Mannequin.Init(m_wRoot);
		m_PresetMenu.Init(this, m_wRoot);
		m_PresetDrag.Init(this, m_wRoot);
	}
	...
	protected void ReleaseBookkeeping()
	{
		...
		// and the modal given back, and the drag pump taken off the queue, before the workspace or
		// the call queue is left holding a widget about to be freed
		m_PresetMenu.Teardown();
		m_PresetDrag.Teardown();
	...
	//------------------------------------------------------------------------------------------------
	//! Three tiers, most recent first: this session's unapplied edits, then the saved kit the class
	//! was last APPLIED from in any session, then the authored defaults. The middle tier is why a
	//! player who saved a kit once does not rebuild it on a fresh server; a saved kit deleted since
	//! falls through to the defaults, which is what the Standard row holds.
	void LoadPicks()
	{
	...
	//------------------------------------------------------------------------------------------------
	RK29_PresetMenu PresetMenu()
	{
		return m_PresetMenu;
	}

	//------------------------------------------------------------------------------------------------
	RK29_PresetDrag PresetDrag()
	{
		return m_PresetDrag;
	}
	...
	protected bool DropBlockedPicks()
	{
		...
		for (int i = m_aPicks.Count() - 1; i >= 0; i--)
		{
			...
			// a bare pick is an answer only where the group offers None - and only while a rule has
			// not withheld it, or the wire would keep saying None while the kit issues the default
			if (pick.m_sEntry == "")
			{
				if (!g.NoneAllowed())
				{
	...
	{
		// whatever the last press was refused for, the picks have moved since - the band is raised
		// again by the next press if it is still true, and a stale refusal must not outlive the state
		// it was about
		HideApplyError();
	...
	//------------------------------------------------------------------------------------------------
	//! EXCLUSIVE groups: one pick per group, so the group's old pick goes first. The count is inert
	//! on such a group, with one exception: a loaded selector reads it as
	//! RK29_KitResolve.LOADED_PICK_EMPTY, and OnLoadedToggle is the only caller that passes it.
	void SetPick(string groupId, string entryId, int count = 1)
	{
		for (int i = m_aPicks.Count() - 1; i >= 0; i--)
		{
	...
	protected void OnModularButtonClicked(SCR_ModularButtonComponent comp)
	{
		...
		// the button comes from the press a moment earlier, which is the only event that carries it -
		// see RK29_LoadoutRowHandler.OnMouseButtonDown. A row with no mouse-aware handler, and a row
		// "clicked" by Enter or a gamepad, both leave the note at LEFT.
		OnRowClicked(handler.m_eKind, handler.m_iIndex, TakeMouseButton());
	}

	//------------------------------------------------------------------------------------------------
	//! AttachHandler for the rows that have to know which mouse button pressed them, or that are
	//! drag handles. The click still arrives the ordinary way, through the component's m_OnClicked
	//! - that path is the one this menu has always used and the only one that is reliable, because
	//! the component moves focus inside its own OnClick and a handler added after it does not get
	//! that event at all on the press that moves focus. The handler is ADDED as well, purely for
	//! the mouse-down and mouse-up the component does not forward; the button is read off the
	//! press and consumed by the click that follows.
	void AttachClickHandler(notnull Widget row, string buttonName,
		RK29_EMenuRowKind kind, int index, notnull array<ref RK29_LoadoutRowHandler> store)
	{
		Widget button = row.FindAnyWidget(buttonName);
		if (!button)
		...
		if (!comp)
		{
			Print(string.Format("[RK29] loadout menu: '%1' carries no modular button - row kind %2"
				+ " is not clickable", buttonName, kind), LogLevel.WARNING);
			return;
		}
	...
	//! RK29_LoadoutRowHandler.OnChange, on commit only, and never arrive here. Only the saved-kit
	//! rows read the button; every other kind is reached through a path that cannot say which it
	//! was and always states the left one.
	void OnRowClicked(RK29_EMenuRowKind kind, int index, int button)
	{
		...
		{
			// a right-click on a saved kit row opens its menu rather than loading it, and Standard
			// has no menu - so here the right button says nothing at all, which is the point: it
			// must not be the one press that silently throws the session's picks away
		...
		{
			// the release that ended a drag raises this click too, and loading the row that was just
			// moved is not what the hand that moved it asked for. Only the left button asks: a drag
			// that moved a row ends on a DIFFERENT row than it began on, so no click is raised at
			// all and the flag is still standing when the next press arrives - and only a left press
			// clears it. Consuming it here for a right-click swallowed the menu instead.
			if (button == MouseState.LEFT && m_PresetDrag.TookClick())
				return;

			if (button == MouseState.RIGHT)
				m_Info.OpenPresetMenu(index);
			else
				m_Info.OnPresetClicked(index);

			return;
		}
		if (kind == RK29_EMenuRowKind.PRESET_MENU_OVERWRITE)
		{
			m_Info.OnPresetMenuOverwrite();
			return;
		}
		if (kind == RK29_EMenuRowKind.PRESET_MENU_DELETE)
			m_Info.OnPresetMenuDelete();
	}

	//------------------------------------------------------------------------------------------------
	//! A button going down under the cursor. Only a saved-kit row is listening: the row is a drag
	//! handle, and holding it is told from clicking it by how far the cursor travels before the
	//! release - see RK29_PresetDrag. The right button is not a handle, or a press meant for the
	//! row's menu would arm a reorder.
	void NoteMouseButton(int button)
	{
		m_iMouseButton = button;
	}

	//------------------------------------------------------------------------------------------------
	//! The noted button, forgotten as it is read: a row clicked with no press behind it - Enter, a
	//! gamepad - must not inherit the last mouse press's button.
	protected int TakeMouseButton()
	{
		int button = m_iMouseButton;
		m_iMouseButton = MouseState.LEFT;
		return button;
	}

	//------------------------------------------------------------------------------------------------
	void OnRowPressed(RK29_EMenuRowKind kind, int index, int button)
	{
		if (kind != RK29_EMenuRowKind.PRESET_ROW || button != MouseState.LEFT)
			return;

		m_PresetDrag.Press(index, m_Info.PresetNameAt(index));
	}

	//------------------------------------------------------------------------------------------------
	//! The release, which ends a drag wherever the cursor has got to. A press that never became one
	//! is left alone here and answered by the click.
	void OnRowReleased(RK29_EMenuRowKind kind, int index, int button)
	{
		if (kind != RK29_EMenuRowKind.PRESET_ROW || button != MouseState.LEFT)
			return;

		m_PresetDrag.Release();
	}
	...
	{
		// The innermost thing open goes first: a saved kit's context menu sits over the band and is
		// not a step through the menu, so it must not cost the player their open group as well.
		// TookBack is the other half of that - while the menu is modal it answers the press through
		// its own listener, and without asking here this same press would go on to fold the detail
		// panel or close the dialog behind it.
		if (m_PresetMenu.Close() || m_PresetMenu.TookBack())
			return true;
	...
	//============================================================================================
	//------------------------------------------------------------------------------------------------
	//! Whether this Apply press must be turned away, and the whole refusal if so: the band naming
	//! every reason and the refusal sound - the confirm button's own click is muted in
	//! RK29_LoadoutDialog.OnMenuOpen so this is the only sound the press makes. Called from
	//! RK29_LoadoutDialog.OnConfirm, BEFORE the dialog's own confirm runs - vanilla's OnConfirm
	//! closes the menu, so a refusal that returned afterwards would be a message on a closing screen.
	//!
	//! Both refusals are client-side, and only the floor is re-read by the server: what does not fit
	//! can only be answered by dressing a body, which the menu has already done for its preview and
	//! the server would have to do a second time to ask the same question.
	bool RefuseApply()
	{
		// a count typed into a row commits on focus loss and rebuilds a frame later - the very press
		// that takes the focus away - so the queued rebuild is pulled forward rather than judged
		// against the answers of the rebuild before it
		GetGame().GetCallqueue().Remove(AfterPickChangedDeferred);
		RefreshAll(false);

		array<string> reasons = {};
		if (!ApplyBlockers(reasons))
			return false;

		ShowApplyError(reasons);
		SCR_UISoundEntity.SoundEvent(SCR_SoundEvent.ACTION_FAILED);
		return true;
	}

	//------------------------------------------------------------------------------------------------
	//! Every reason this kit cannot be issued, one line each, in the order a player would fix them:
	//! the pools that are short first - they are answered in the picker - then what the body could
	//! not carry. One item per line either way; a comma-joined list of six magazines is unreadable.
	protected bool ApplyBlockers(notnull array<string> outReasons)
	{
		// by id: a class carrying two grenade launchers offers ugl_grenades twice, and picks are keyed
		// by group id, so both copies are the same shortfall said once
		array<string> named = {};
		foreach (RK29_ResolvedGroup g : m_aOffer)
		{
			if (!g || named.Contains(g.m_sId))
				continue;

			int spend = RK29_MenuRowKit.GroupSpendOf(g, m_aPicks);
			if (!RK29_KitResolve.UnderMin(g, spend))
				continue;

			named.Insert(g.m_sId);
			outReasons.Insert(string.Format("%1: needs at least %2 supplies, has %3", g.m_sDisplayName,
		...
		RK29_MenuRowKit.DroppedLines(m_Mannequin.Dropped(), dropped);
		foreach (string line : dropped)
			outReasons.Insert("Does not fit: " + line);

		TrimReasons(outReasons);
		return !outReasons.IsEmpty();
	}

	//------------------------------------------------------------------------------------------------
	//! The band grows with its lines and stands over the columns, so a kit that drops a dozen items
	//! would bury the picker it is telling the player to go back to. The tail is counted instead;
	//! the weight row's own tip still names every one of them.
	protected void TrimReasons(notnull array<string> reasons)
	{
		if (reasons.Count() <= APPLY_ERROR_MAX_LINES)
			return;

		int hidden = reasons.Count() - APPLY_ERROR_MAX_LINES;
		while (reasons.Count() > APPLY_ERROR_MAX_LINES)
			reasons.RemoveOrdered(reasons.Count() - 1);

		reasons.Insert(string.Format("...and %1 more", hidden));
	}

	//------------------------------------------------------------------------------------------------
	protected void ShowApplyError(notnull array<string> reasons)
	{
		if (!m_wApplyError)
			return;

		string text;
		foreach (int i, string reason : reasons)
		{
			if (i > 0)
				text += "\n";
			text += reason;
		...
		RK29_WidgetUtil.SetText(m_wApplyError, "ApplyErrorTitle", APPLY_ERROR_TITLE);
		RK29_WidgetUtil.SetText(m_wApplyError, "ApplyErrorText", text);
		m_wApplyError.SetVisible(true);
	}

	//------------------------------------------------------------------------------------------------
	protected void HideApplyError()
	{
		if (m_wApplyError)
			m_wApplyError.SetVisible(false);
	}

...
class RK29_KitResolve
{
	...
	static RK29_ResolvedEntry ExclusiveAnswer(notnull RK29_ResolvedGroup g, array<ref RK29_ChoicePick> picks)
	{
		...
		if (g.NoneAllowed() && bare && bare.m_sEntry == "")
			return null;
		...
		array<ref RK29_ChoicePick> picks, RK29_KitComposition comp, bool weaponTargets)
	{
		...
		foreach (RK29_Exclusion exclusion : comp.m_aExclusions)
		{
			...
			int have = 0;
			if (whenEntry == NONE_ENTRY)
			{
				// "the group is answering None". Asked of the same function the apply reads, so a
				// rule and the kit can never disagree about what was answered. A withheld None
				// makes the answer real, so this correctly stops firing.
				RK29_ResolvedEntry answered;
				if (source.IsWeaponGroup())
					answered = PickedWeaponEntry(source, picks);
				else
					answered = ExclusiveAnswer(source, picks);

				if (!answered && source.NoneAllowed())
					have = 1;
			}
			else if (source.m_eKind == RK29_EChoiceKind.EXCLUSIVE || source.IsWeaponGroup())
			{
				...
				if (chosen && !chosen.m_bBlocked && TargetReaches(source, chosen, whenGroup, whenEntry))
					have = 1;
			}
			else
			{
				...
				{
					if (!candidate || !TargetReaches(source, candidate, whenGroup, whenEntry))
						continue;
			...
			foreach (RK29_ResolvedEntry trigger : source.m_aEntries)
			{
				if (!trigger || trigger.m_sExcludes != "")
					continue;
				if (!TargetReaches(source, trigger, whenGroup, whenEntry))
					continue;
			...
			// None is withheld only when NAMED. A bare "<group>" block rules out the group's rows
			// and leaves the bare answer standing, which is what makes "force None" spellable as
			// blocking everything - and stops a blanket block from silently leaving a group with no
			// legal answer at all.
			if (blockEntry == NONE_ENTRY)
			{
				target.m_bNoneBlocked = true;
				continue;
			}
			...
			foreach (RK29_ResolvedEntry victim : target.m_aEntries)
			{
				if (!victim || victim.m_bBlocked)
					continue;
				if (!TargetReaches(target, victim, blockGroup, blockEntry))
					continue;
	...
	//! Reserved entry id addressing a group's None answer from an exclusion - "<group>/none". No
	//! entry ever carries it: None is not a row in m_aEntries, so a block against it sets
	//! RK29_ResolvedGroup.m_bNoneBlocked instead. Note the near-miss spelling: "optics_none" is a
	//! GROUP id (an entry-less placeholder substituted in to deny a seat), one character of
	//! punctuation from "optics/none", which is this. The lint refuses the id on a group that
	//! allows no None, so a typo cannot read as a working rule.
	static const string NONE_ENTRY = "none";
	...
	protected static void CopyGroupShape(notnull RK29_ChoiceGroup def, notnull RK29_ResolvedGroup g)
	{
		...
		if (itemDef)
		{
			...
			g.m_iBudgetMin = itemDef.m_iBudgetMin;
			ClampBudgetMin(g);
		...
		// EXCLUSIVE is already the zero value of m_eKind, so the empty flag is all a weapon group has
		// to copy
		RK29_WeaponGroup weaponDef = RK29_WeaponGroup.Cast(def);
		if (weaponDef)
		{
			g.m_eGroupType = RK29_EGroupType.WEAPON;
			g.m_bAllowEmpty = weaponDef.m_bAllowEmpty;
		}
	}
	...
	//! the seated magazine from the ammo group. Null when the ammo group offers no items. The mark
	//! is never stored against a row - SeatedLoadedEntry derives which row wears it - so a pick here
	//! is a preference, not the answer. Reads the resolved ammo group, so it needs no faction or
	//! parked filtering of its own.
	...
	protected static void DropEmptyGroups(notnull array<ref RK29_ResolvedGroup> groups)
	{
		for (int i = groups.Count() - 1; i >= 0; i--)
		{
			...
			// m_bAllowEmpty as AUTHORED, not NoneAllowed(): such a group exists to state that the
			// slot goes empty, and a rule withholding its only answer is a config error the lint
			// names - dropping the group here would silently undo the eviction it was written for
		...
		if (p.m_aAdjust)
		{
			foreach (RK29_OverrideAdjust adj : p.m_aAdjust)
			{
				...
				if (entryId == "")
				{
					...
					if (adj.m_iBudgetMin >= 0)
						g.m_iBudgetMin = adj.m_iBudgetMin;
					// either number may have moved, and a kit that lowered only the budget must not be
					// left with a floor above it
					ClampBudgetMin(g);
					continue;
				}
	...
	//------------------------------------------------------------------------------------------------
	//! Whether an exclusion path reaches this entry of the group FindGroupOwnedBy answered for it. The
	//! group's own id reaches every entry; an included id reaches only what that include contributed,
	//! so a family merged into a wider slot is blocked without its siblings. Public because the menu's
	//! note must name the same rows EnforceExclusions blocks.
	static bool TargetReaches(notnull RK29_ResolvedGroup g, notnull RK29_ResolvedEntry e,
		string groupId, string entryId)
	{
		if (entryId != "" && e.m_sId != entryId)
			return false;
		return g.m_sId == groupId || e.m_sFromGroup == groupId;
	}
	...
	static RK29_ResolvedEntry PickedEntry(notnull RK29_ResolvedGroup group, array<ref RK29_ChoicePick> picks)
	{
		...
		if (pick)
		{
			if (pick.m_sEntry == "" && group.NoneAllowed())
				return null;
	...
	//!
	//! A chamber whose every row is at zero goes out with an empty prefab, since emitting no pick at
	//! all means "leave the prefab default" - the one case SeatedLoadedEntry has nowhere to move the
	//! mark to, and so the only way a kit fields an empty chamber. Appended,
		...
		notnull map<string, int> weaponSlots, notnull map<int, ref array<ref RK29_LoadedPick>> outLoadedMags)
	{
		...
		// SeatedLoadedEntry ends in the group's default and a synthesized selector never allows a
		// bare answer, so null means the selector has nothing left to chamber. It is also what moves
		// the mark off a row the kit stopped carrying, so the empty-chamber branch below is reached
		// only when every row feeding this chamber is at zero
		RK29_ResolvedEntry chosen = SeatedLoadedEntry(groups, g, picks);
		if (!chosen)
			return;
	...
	//------------------------------------------------------------------------------------------------
	//! Does this spend fall short of the group's floor? False for anything that is not BUDGETED and
	//! for a group that authored none.
	//!
	//! Not a clamp anywhere, and deliberately so: unlike overspending, a short group is left as the
	//! player set it - the counts stand, the preview wears them, and only Apply refuses (see
	//! RK29_LoadoutMenu.RefuseApply). Resetting it to defaults on the way through would fight the
	//! player mid-trade, since emptying one row to fill another passes through short.
	static bool UnderMin(notnull RK29_ResolvedGroup g, int spend)
	{
		return g.m_eKind == RK29_EChoiceKind.BUDGETED && g.m_iBudgetMin > 0
			&& spend < g.m_iBudgetMin;
	}

	//------------------------------------------------------------------------------------------------
	//! A floor above the ceiling is a group nothing could ever satisfy, so the ceiling wins. Said
	//! here once for both the authored pair and whatever an override leaves behind; the lint
	//! complains about the authoring separately, this only keeps the runtime answerable.
	static void ClampBudgetMin(notnull RK29_ResolvedGroup g)
	{
		if (g.m_iBudgetMin > g.m_iBudget)
			g.m_iBudgetMin = g.m_iBudget;
	}

	//------------------------------------------------------------------------------------------------
	//! The first group of this class's offer whose picks fall short of its floor, by id, or "" when
	//! every pool is satisfied. The server's own reading of what the picker refuses to send: the
	//! offer is rebuilt here from the class and the wire, so a hand-made request is measured against
	//! the same numbers the menu measured.
	static string FirstUnderMinGroup(RK29_ClassSetup cls, RK29_KitSetup setup,
		array<ref RK29_ChoicePick> picks)
	{
		if (!cls || !setup)
			return "";
		...
		{
			if (g && UnderMin(g, SpendOf(g, picks)))
				return g.m_sId;
		}
		return "";
	}
	...
	//------------------------------------------------------------------------------------------------
	//! Which row of a chamber's selector the mark stands on - the one read by the menu, by the cargo
	//! deduction and by the seat order alike, so the three cannot disagree about what is in the gun.
	//!
	//! The picked row while the kit carries any of it. Carrying none of it moves the mark to the
	//! most-carried row on this chamber rather than sending the gun out empty beside full pouches;
	//! ties go to the row listed first. With every row on the chamber at zero there is nowhere to
	//! move, so the mark stays where it is and the chamber is empty - and it re-homes by itself the
	//! moment one of those rows is raised, because the answer is derived on every read and never
	//! written back as a pick.
	//!
	//! The exception is the chamber the player asked to leave empty - LOADED_PICK_EMPTY - which is
	//! never moved. Stored state cannot tell a mark PUT on an empty row from a mark the counts were
	//! emptied UNDER, and those are opposite instructions, so the menu decides it at the click and
	//! writes the answer into the pick.
	static RK29_ResolvedEntry SeatedLoadedEntry(notnull array<ref RK29_ResolvedGroup> groups,
		notnull RK29_ResolvedGroup loadedGroup, array<ref RK29_ChoicePick> picks)
	{
		RK29_ResolvedEntry seated = PickedEntry(loadedGroup, picks);
		if (!seated)
			return null;

		if (CarriedCountOfLoaded(groups, loadedGroup, seated, picks) > 0)
			return seated;

		// read against the seated row's own id: a pick naming an entry the offer no longer has falls
		// back to the default, and the flag must not travel with it onto a row nobody marked
		RK29_ChoicePick pick = FindPick(picks, loadedGroup.m_sId);
		if (pick && pick.m_iCount == LOADED_PICK_EMPTY && pick.m_sEntry == seated.m_sId)
			return seated;

		RK29_ResolvedEntry fullest = FullestLoadedEntry(groups, loadedGroup, picks);
		if (fullest)
			return fullest;
	...
	//------------------------------------------------------------------------------------------------
	//! m_iCount is inert on an EXCLUSIVE group's pick, and a loaded selector spends it as a flag: 0
	//! is a mark the player put on a row carried zero times while the chamber had stock elsewhere -
	//! a chamber deliberately left empty, which SeatedLoadedEntry then never re-homes. It rides the
	//! wire, since EncodePicks writes any count that is not 1, so a preset keeps an empty chamber
	//! empty. Written by RK29_MenuDetailPanel.OnLoadedToggle and nowhere else.
	//!
	//! Only while that row is still at zero: stocking the row the flag names seats it like any
	//! other, and emptying it again leaves it empty again. The flag is about one row, not the gun.
	static const int LOADED_PICK_EMPTY = 0;

	//------------------------------------------------------------------------------------------------
	//! The most-carried row on one chamber, null when every row feeding it is at zero. Ties go to
	//! the row listed first: the selector's entry order is the row order the player reads. Also the
	//! menu's test for "this chamber has ammo somewhere", which is what separates a mark put on an
	//! empty row from a mark put on a chamber that is empty whatever it names.
	static RK29_ResolvedEntry FullestLoadedEntry(notnull array<ref RK29_ResolvedGroup> groups,
		notnull RK29_ResolvedGroup loadedGroup, array<ref RK29_ChoicePick> picks)
	{
		RK29_ResolvedEntry fullest = null;
		int most = 0;
		foreach (RK29_ResolvedEntry e : loadedGroup.m_aEntries)
		{
			// a blocked row is carried zero times anyway, but CarriedCountOfLoaded answers 1 for a
			// row no group states a total for, and a blocked row must not take the chamber on that
			if (!e || e.m_bBlocked)
				continue;

			int carried = CarriedCountOfLoaded(groups, loadedGroup, e, picks);
			if (carried > most)
			{
				most = carried;
				fullest = e;
	...
	//! row - nothing says the round is not carried, so it seats. Public for the menu's sake: the
	//! LOADED toggle has to know whether the row under the cursor is carried before it writes.
	static int CarriedCountOfLoaded(notnull array<ref RK29_ResolvedGroup> groups,
		...
		array<ref RK29_ChoicePick> picks, notnull RK29_KitSetup setup, ResourceName ownerPrefab)
	{
		...
		if (loadedGroup)
		{
			RK29_ResolvedEntry seated = SeatedLoadedEntry(groups, loadedGroup, picks);
			if (seated)
			{
	...
	//! as Apply records it. A group is absent wherever Apply would have skipped it, unless
	//! includeBareAnswers says otherwise - see the second pass.
		...
		bool includeBareAnswers = false)
	{
		...
		MapBareWeaponGroups(groups, picks, setup, factionKey, includeBareAnswers, claimed, outSlots);
	}

	//------------------------------------------------------------------------------------------------
	//! DISPLAY ONLY: the slot a group answering None still speaks for. A weapon group that allows
	//! None claims nothing when it is answered that way, which would cost the picker the caption
	//! naming the slot ("SIDEARM") and sort the row into the slotless sink at the bottom. The seat
	//! its own entries would have taken stands in.
	//!
	//! Runs after every real claim and never touches outWeaponIdSlots: no weapon is in the kit, so a
	//! bare group must not take the slot a gun actually fills, and nothing keyed by weapon id may
	//! learn of one that was never issued.
	protected static void MapBareWeaponGroups(notnull array<ref RK29_ResolvedGroup> groups,
		array<ref RK29_ChoicePick> picks, notnull RK29_KitSetup setup, string factionKey,
		bool includeBareAnswers, notnull array<int> claimed, notnull map<string, int> outSlots)
	{
		if (!includeBareAnswers)
			return;

		foreach (RK29_ResolvedGroup g : groups)
		{
			if (!g || !g.IsWeaponGroup() || !g.NoneAllowed())
				continue;
			if (outSlots.Contains(g.m_sId))
				continue;

			RK29_ResolvedEntry stand = g.FirstUnblockedEntry();
			if (!stand)
				continue;
			RK29_EntryWeapon weaponEntry = RK29_EntryWeapon.Cast(stand.m_Def);
			if (!weaponEntry)
				continue;

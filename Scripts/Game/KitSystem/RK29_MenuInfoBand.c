...
class RK29_MenuInfoBand
{
	...
	//! What a saved kit answers to, said once on the caption rather than on every row: the rows
	//! stay silent while nothing is wrong with them, and none of these three has anything on the
	//! row to advertise it.
	//!
	//! Authored as its own lines, which AttachHoverTip is asked to keep - and keeping them means
	//! NOT wrapping them, so each one has to fit RK29_HoverTip.TIP_CHARS_PER_LINE by hand. A
	//! longer line runs past the panel and is clipped at the right edge; these are 30, 29 and 25.
	protected static const string PRESETS_HINT = "Left-click a kit to select it."
	...
	//! "<kit>|<name>" of the preset the player last put on screen, and ONLY a tie-break: which
	//! row lights is still computed from the picks, so nudging a count after a load still lights
	//! nothing. It settles the case the wire cannot - several presets holding the SAME picks, which
	//! is what saving a loaded kit under a second name produces - where matching alone pointed at
	//! whichever was saved first and clicking any of the others appeared to do nothing.
	//! Keyed by kit as well as name: two classes may both hold a preset called "AT loadout".
	protected string m_sChosenPreset;

	//! The widget the context menu hangs under, one per booked preset row and in the same order.
	//! Held so the glyph and a right-click anywhere on the row can both put the panel in one place
	//! without walking back up from whatever was pressed. Weak handles into the stamped tree: they
	//! are cleared and refilled by the same rebuild that frees the rows.
	...
	void BuildInfoPanel()
	{
		...
		// both hold preset rows this rebuild is about to free: the context menu is anchored to one,
		// and a drag in flight has faded one and hit-tests the cursor against all of them
		m_Menu.PresetMenu().Close();
		m_Menu.PresetDrag().Cancel();
	...
	{
		array<string> lines = {};
		RK29_MenuRowKit.DroppedLines(dropped, lines);
	...
	protected void BuildPresetSection(notnull RK29_ClassSetup cls)
	{
		...
		// Standard lights only when nothing saved claims the wire.
		int lit = -1;
		foreach (int i, RK29_KitPreset candidate : presets)
		{
			...
			// the first match is only the fallback now: it is the right answer when the player has
			// not chosen among equals, and the wrong one the moment they have
			if (lit < 0)
				lit = i;

			if (ChosenKey(cls, candidate.m_sName) == m_sChosenPreset)
			{
				lit = i;
				break;
			}
		...
		// the caption answers the cursor where the rows cannot: a healthy kit is silent, and two of
		// the three things it answers to have nothing on the row to advertise them
		Widget caption = RK29_MenuRowKit.StampHeader(m_wColInfo, PRESETS_HEADER);
		if (caption)
	...
	protected void StampStandardRow(string currentWire, bool savedKitLit)
	{
		...
		// the click path, not the ordinary one: this row throws the session's picks away, and the
		// right button has to be refused here rather than be the press that silently does it
		m_Menu.AttachClickHandler(row, "RowButton", RK29_EMenuRowKind.PRESET_STANDARD, 0,
			m_aInfoHandlers);
	}

	//------------------------------------------------------------------------------------------------
	//! One saved preset: its name, what is wrong with it, and the glyph that opens its menu. A left
	...
	protected void StampPresetRow(notnull RK29_KitPreset preset, bool lit)
	{
		...
		int index = PushPresetRow(preset.m_sName, row);
		...
		// because the left button loads, the right one opens the row's menu, and a hold drags it
		m_Menu.AttachClickHandler(row, "RowButton", RK29_EMenuRowKind.PRESET_ROW, index,
			m_aInfoHandlers);
	}
	...
	//! invariant as PushDetailRow. Both of a preset row's handlers, the load click and the menu
	//! glyph, carry this one index. The Standard row and the save row book nothing: neither stands
	//! for a preset. The anchor is the row root, not its content line: the drag fades the anchor to
	//! say which row is in the air, and RowBg - the amber plate that says a row is the loaded kit -
	//! is a SIBLING of RowLine, so fading the line left a dragged selection reading as fully lit
	//! with its own ghost beside it. The root covers plate and content together.
	protected int PushPresetRow(string name, notnull Widget row)
	{
		m_aPresetRows.Insert(name);
		m_aPresetAnchors.Insert(row);
	...
	//------------------------------------------------------------------------------------------------
	//! The tie-break key for one preset of one class. "" for a class with no kit name, which can
	//! never equal a stored key - so a half-built class ties nothing rather than tying everything.
	protected static string ChosenKey(RK29_ClassSetup cls, string name)
	{
		if (!cls || cls.m_sKitName == "" || name == "")
			return "";

		return cls.m_sKitName + "|" + name;
	}

	//------------------------------------------------------------------------------------------------
	//! null for an index this section no longer has, on the same terms as PresetName; the two arrays
	//! are filled and cleared together, so one answering means the other does.
	Widget PresetAnchor(int index)
	{
		if (index < 0 || index >= m_aPresetAnchors.Count())
			return null;

		return m_aPresetAnchors[index];
	}

	//------------------------------------------------------------------------------------------------
	//! The booked rows themselves, in list order - what RK29_PresetDrag hit-tests the cursor
	//! against and hangs its drop line on. Handed out live rather than copied: the drag reads it
	//! every tick, and a rebuild has to be able to empty it under one in flight.
	array<Widget> PresetAnchors()
	{
		return m_aPresetAnchors;
	}

	//------------------------------------------------------------------------------------------------
	//! The name a booked row stands for, for a caller that has an index and needs the name that
	//! outlives it.
	string PresetNameAt(int index)
	{
	...
	void OnPresetClicked(int index)
	{
		...
		string chosen = ChosenKey(cls, name);
		...
		if (RK29_KitResolve.EncodePicks(m_Menu.Picks()) == preset.m_sPicks)
		{
			...
			if (m_sChosenPreset == chosen)
				return;

			m_sChosenPreset = chosen;
			BuildInfoPanel();
			return;
		}

		// set before the rebuild, which is inside AfterPickChanged: the row that lights is chosen
		// in there, and this is what tells it which of several identical wires the player meant
		m_sChosenPreset = chosen;
	...
	//------------------------------------------------------------------------------------------------
	//! The row's glyph, and a right-click anywhere on the row, both land here: the overwrite/delete
	//! menu, hung under that row. Opened by name rather than by index - the menu outlives the
	//! rebuild that would invalidate the index, and by then only the name still says which preset.
	void OpenPresetMenu(int index)
	{
		string name = PresetName(index);
		if (name == "")
			return;

		m_Menu.PresetMenu().Open(PresetAnchor(index), name);
	}

	//------------------------------------------------------------------------------------------------
	//! A dragged row let go over a new place in the list. The store persists the new order itself,
	//! so only the band is restamped: nothing about the picks moved, and which row is lit is
	//! computed from the picks rather than remembered, so the highlight follows the row it belongs
	//! to without being told.
	void OnPresetDropped(string name, int newPos)
	{
		if (name == "")
			return;
		...
		RK29_KitPresetStorage store = RK29_KitPresetStorage.GetInstance();
		if (!store || !store.Reorder(cls.m_sKitName, name, newPos))
			return;

		RK29_Log.Trace(string.Format("[RK29] loadout menu: moved preset '%1' of %2 to position %3",
			name, cls.m_sKitName, newPos));

		// Next frame, for the reason OnPresetNameCommitted defers its own: the rebuild frees every
		// preset row, and this one runs inside a mouse-up dispatch that is not finished with them.
		// A release raises button-up on the widget under the cursor AND on the widget the press
		// began on (ScriptedWidgetEventHandler.OnMouseButtonUp), which for a completed drag are two
		// different rows - so a synchronous rebuild frees the second one before it is dispatched to.
		GetGame().GetCallqueue().CallLater(BuildInfoPanelDeferred, 0, false);
	}

	//------------------------------------------------------------------------------------------------
	//! The open menu's Overwrite: the named preset is saved over with the picks on screen, keeping
	//! its name and its place in the list - see RK29_KitPresetStorage.Save. The name is taken before
	//! the close, which clears it. No confirmation, for the reason OnPresetClicked states: the row
	//! it overwrites is one click from being saved again.
	void OnPresetMenuOverwrite()
	{
		RK29_PresetMenu menu = m_Menu.PresetMenu();
		string name = menu.Target();
		menu.Close();
		if (name == "")
			return;
		...
		RK29_Log.Trace(string.Format("[RK29] loadout menu: overwrote preset '%1' of %2 picks='%3'",
			name, cls.m_sKitName, wire));
		RememberSavedAsWorn(cls, name, wire);
		m_sChosenPreset = ChosenKey(cls, name);
		BuildInfoPanel();
	}

	//------------------------------------------------------------------------------------------------
	//! The open menu's Delete. The store persists the removal itself, so only the band is restamped:
	//! nothing about the picks moved, so the columns, the offer and the mannequin are all still
	//! right.
	void OnPresetMenuDelete()
	{
		RK29_PresetMenu menu = m_Menu.PresetMenu();
		string name = menu.Target();
		menu.Close();
	...
	void OnPresetNameCommitted(Widget w)
	{
		...
		RememberSavedAsWorn(cls, typed, wire);

		// the kit just saved is the one on screen, and it is saved from picks some OTHER preset may
		// already hold - without this the new row is stamped dark while its older twin keeps the
		// plate. The typed name is the stored name: the box caps its own length at MAX_NAME_LENGTH,
		// so Save's cap cannot shorten it further and only the trim can move it.
		m_sChosenPreset = ChosenKey(cls, typed);
	...
	protected void RememberSavedAsWorn(notnull RK29_ClassSetup cls, string name, string wire)
	{
		...
		if (store)
			store.MarkPreset(cls.m_sKitName, name);
	}
}

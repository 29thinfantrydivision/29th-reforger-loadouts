//------------------------------------------------------------------------------------------------
//! What one stamped row's click means; AttachHandler and AttachClickHandler name one and
//! RK29_LoadoutMenu.OnRowClicked dispatches on it - except COUNT_EDIT and PRESET_SAVE_EDIT, the two
//! edit boxes, which are dispatched from RK29_LoadoutRowHandler.OnChange on commit and never reach
//! OnRowClicked.
enum RK29_EMenuRowKind
{
	...
	PRESET_STANDARD,
	//! a saved kit. Left-click loads it, right-click opens its overwrite/delete menu, and holding
	//! it drags it up or down the list - which is why it is attached by AttachClickHandler.
	PRESET_ROW,
	//! the two items of that menu - neither carries an index, the open menu holds the preset name
	PRESET_MENU_OVERWRITE,
	PRESET_MENU_DELETE,
...
//------------------------------------------------------------------------------------------------
//! The three numbers a budgeted caption carries: what the picks spend, between the bounds it is
//! allowed to sit between. The spend keeps the supplies glyph beside it - the unit is what marks
//! which of the three is the live number - and the bounds are annotations on it.
class RK29_HeaderSupply
{
	int m_iSpend;

	//! 0 = no floor authored; the left bound and its pipe are left out entirely rather than shown
	//! as a zero nobody has to reach
	int m_iMin;
...
class RK29_MenuRowKit
{
	...
	//! The ink a budgeted caption's bounds are written in, and the three weights BoundAlpha picks
	//! between. The faintest is still a number to be read at a glance, not a watermark - a bound
	//! nobody can make out is a bound that may as well not be drawn.
	protected static const int BOUND_INK_R = 189;
	protected static const int BOUND_INK_G = 189;
	protected static const int BOUND_INK_B = 188;
	protected static const int BOUND_ALPHA_ON = 255;
	protected static const int BOUND_ALPHA_NEAR = 209;
	protected static const int BOUND_ALPHA_FAR = 158;
	...
	//!
	//! The SUPPLY flag opens the header's right-hand zone: the spend with the supplies glyph beside
	//! it, walled off by pipes from the bounds either side; the section's name keeps the left.
	//! KitHeader's HeightOverride keeps the larger type from growing the header, so it has to fit
	//! 26 pixels. `supply` is ignored without the flag, and the flag without it stamps nothing.
	//! Answers the caption's HOVER TARGET so a section that wants to say more can hang a tip on it,
	//! null when nothing was stamped; every other caller ignores it. The target is HeaderButton and
	//! not the row: only a ButtonWidget answers the cursor here. Layout and text widgets raise no
	//! mouse enter at all, and neither does a transparent image - both were tried. Every hover tip
	//! in this menu that works is on a button (RK29_TraitRow and RK29_PresetRow both wrap their
	//! content in one), which is why HEADER_LAYOUT now wraps its line the same way.
	static Widget StampHeader(Widget parent, string caption,
		RK29_EHeaderFlags flags = RK29_EHeaderFlags.NONE, RK29_HeaderSupply supply = null)
	{
		if (!parent)
			return null;
		...
		// glyph and the numbers cannot come up half-drawn on a header that asked for neither
		if ((flags & RK29_EHeaderFlags.SUPPLY) && supply)
		{
			Widget zone = row.FindAnyWidget("HeaderSupplyZone");
			if (zone)
				zone.SetVisible(true);

			StampSupplyNumbers(row, supply);
		}

		if (flags & RK29_EHeaderFlags.LOCK)
			RK29_WidgetUtil.RevealPair(row, "HeaderLockSize", "HeaderLockIcon", true);

		return row.FindAnyWidget("HeaderButton");
	}

	//------------------------------------------------------------------------------------------------
	//! The budgeted caption's numbers: the spend in full white beside its glyph, each bound drawn as
	//! loudly as it is currently relevant. A floor the picks have not reached is the one thing that
	//! refuses an Apply, so it goes red and stays red - the spend never does, because the spend is
	//! not what is wrong.
	//!
	//! The left bound and its pipe stay hidden on a group with no floor: it reads "4 [glyph] | 5",
	//! which is what the caption said before any of this existed.
	protected static void StampSupplyNumbers(notnull Widget row, notnull RK29_HeaderSupply supply)
	{
		RK29_WidgetUtil.SetText(row, "HeaderTotal", supply.m_iSpend.ToString());
		RK29_WidgetUtil.SetText(row, "HeaderMax", supply.m_iMax.ToString());
		TintBound(row, "HeaderMax", BoundAlpha(supply.m_iSpend, supply.m_iMax, supply.m_iMax));

		bool hasFloor = supply.m_iMin > 0;
		Widget pipe = row.FindAnyWidget("HeaderPipeLeft");
		if (pipe)
			pipe.SetVisible(hasFloor);

		Widget minText = row.FindAnyWidget("HeaderMin");
		if (minText)
			minText.SetVisible(hasFloor);

		if (!hasFloor)
			return;

		RK29_WidgetUtil.SetText(row, "HeaderMin", supply.m_iMin.ToString());
		if (supply.m_iSpend < supply.m_iMin)
			TintWidget(row, "HeaderMin", UIColors.WARNING);
		else
			TintBound(row, "HeaderMin", BoundAlpha(supply.m_iSpend, supply.m_iMin, supply.m_iMax));
	}

	//------------------------------------------------------------------------------------------------
	//! How loudly a bound is drawn: full while the spend stands on it, a step down while it is
	//! within a quarter of the budget, faint beyond that - but never faint enough to have to be
	//! hunted for. Three steps rather than a ramp, so a stepper press either changes the look or
	//! leaves it alone; a number crawling through a gradient shimmers on every press.
	protected static int BoundAlpha(int spend, int bound, int budget)
	{
		int distance = Math.AbsInt(spend - bound);
		if (distance == 0)
			return BOUND_ALPHA_ON;

		if (distance <= Math.MaxInt(1, budget / 4))
			return BOUND_ALPHA_NEAR;

		return BOUND_ALPHA_FAR;
	}

	//------------------------------------------------------------------------------------------------
	protected static void TintBound(notnull Widget row, string widgetName, int alpha)
	{
		TintWidget(row, widgetName, Color.FromSRGBA(BOUND_INK_R, BOUND_INK_G, BOUND_INK_B, alpha));
	}

	//------------------------------------------------------------------------------------------------
	protected static void TintWidget(notnull Widget row, string widgetName, Color ink)
	{
		TextWidget text = TextWidget.Cast(row.FindAnyWidget(widgetName));
		if (text)
			text.SetColor(ink);
	}

	//------------------------------------------------------------------------------------------------
	//! What a dress could not place, folded to one line per item - "2x Bandage" - in drop order.
	//! Shared by the weight row's tip and the refusal under the Apply button, so the two cannot come
	//! to different readings of the same overflow.
	static void DroppedLines(array<ResourceName> dropped, notnull array<string> outLines)
	{
		outLines.Clear();
		if (!dropped)
			return;
		...
		foreach (string label : order)
			outLines.Insert(string.Format("%1x %2", counts.Get(label), label));
	}
	...
	static int SelectableCount(notnull RK29_ResolvedGroup g)
	{
		...
		// a withheld None is not one of the answers either: the tile must not promise it, and a
		// group left on its last real answer padlocks like any other
		if (g.NoneAllowed())
			count++;
	...
	//! one included - a zeroed row loses the LOADED mark to a stocked one unless it was marked empty
	//! on purpose. Named on the
